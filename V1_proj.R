...
## TEMPERATURE AS A CONTINOUS VARIABLE
station_use = function(month, day, hour, station){
  station_demand = 0  
  for (i in 1:n){
    if (station_start[i]== station || station_end[i]== station){
      if (starttime[i,2] == month && starttime[i,3] == day){
        if (starttime[i,4]>=hour && endtime[i,4]<=hour){
          station_demand = station_demand+1
        }
      }
    }
  }
...
#sample week will test for another sample week
sample_month = 6
sample_monday = 6
sample_sunday = 12
demands = c()
...
for (station in stations){
  for (d in sample_monday:sample_sunday = 12){
    for (i in 0:23) {
      demands <- append(demands, station_use(sample_month, d, i, station))
    }
  }
}

model = lm(demands~Temperature)

p_values = c()

for (station in stations){
  as.numeric(stations[i])
  new_p_value = tidy(model)[2][4]
  p_values <- append(p_values, new_p_value)
}


## TEMPERATURE AS A BINARY VARIABLE
